...
document.addEventListener("DOMContentLoaded", function () {
  ...
  tabs.forEach((tab) => {
    tab.addEventListener("click", () => {
      ...
      // Hide all tab contents (tables and summaries)
      tables.forEach((table) => table.classList.add("hidden"));

      // Show the target table
      const targetId = tab.getAttribute("data-target");
      const targetTable = document.getElementById(targetId);
      const targetSummary = document.getElementById(`${targetId}-summary`);
...
document.addEventListener("DOMContentLoaded", function () {
    const loginLink = document.querySelector(".login-link.desktop-only");
    const logoutLink = document.querySelector(".logout-link.desktop-only");

    // If user logs out, update UI immediately
    document.addEventListener("click", (e) => {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const cardViewBtn = document.getElementById('payslipCardViewBtn');
    const tableViewBtn = document.getElementById('payslipTableViewBtn');
    const cardView = document.getElementById('payslipCardView');
    const tableView = document.getElementById('payslipTableView');
    ...
    searchInput.addEventListener('input', () => {
        ...
        cards.forEach(card => {
            ...
            if (month.includes(filter) || year.includes(filter)) {
                card.style.display = '';
            } else {
    ...
    yearFilter.addEventListener('change', () => {
        ...
        cards.forEach(card => {
            const year = card.dataset.year.toLowerCase();
            card.style.display = (selectedYear === 'all' || year === selectedYear) ? '' : 'none';
        });

        rows.forEach(row => {
            const year = row.dataset.year.toLowerCase();
            row.style.display = (selectedYear === 'all' || year === selectedYear) ? '' : 'none';
        });
    });
